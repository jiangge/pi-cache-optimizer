...
本版本保留原有 DeepSeek 行为，并增加针对 Pi 或 provider 能安全暴露 usage 的只读统计 adapter：
...
| OpenAI official | provider 明确是官方 OpenAI/ChatGPT（`openai`、`openai-codex`、`chatgpt`） | `OpenAI cache` | Pi 归一化 usage，或可见 raw 字段 `prompt_tokens_details.cached_tokens` / `input_tokens_details.cached_tokens` 及 prompt/input total |
| Anthropic / Claude | provider/id/name 包含 `anthropic` 或 `claude`，Anthropic API，或 Anthropic cache-control compat | `Claude cache` | Pi 归一化 usage，或可见 raw 字段 `cache_read_input_tokens`、`cache_creation_input_tokens`、`input_tokens` |
| Gemini / Vertex | Google/Gemini/Vertex provider、API、id 或 name 信号 | `Gemini cache` | Pi 归一化 usage，或可见 Gemini/Vertex cached-content token metadata |

Generic OpenAI-compatible 代理**不会**仅因为使用 OpenAI 形状 API 就被当作官方 OpenAI。如果上游 provider 不明确，扩展会隐藏底部统计，而不是猜测。
...
安装后 `PI_CACHE_RETENTION=long` **自动生效**，system prompt **自动重组**，受支持 provider family 的响应完成且暴露 usage 后，底部状态栏会显示缓存统计。
...
- OpenAI prompt caching 对受支持的官方 OpenAI 模型和足够长的 prompt 自动生效。adapter 刻意保守，不会把 generic OpenAI-compatible 代理当作官方 OpenAI。
...
- Proxies/aggregators 可能把同一个 model name 路由到不同上游 provider。请固定上游 routing，并验证 exposed usage 后再判断缓存行为。
